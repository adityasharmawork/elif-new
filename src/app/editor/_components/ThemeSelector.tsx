...
const ThemeSelector = ({ fullWidth = false }: ThemeSelectorProps) => {
  ...
  return (
    <div className={`relative ${fullWidth ? 'w-full' : ''}`} ref={dropdownRef}>
      <motion.button
        ...
        whileTap={{ scale: fullWidth ? 0.98 : 0.98 }}
        onClick={() => setIsOpen(!isOpen)}
        className={`${fullWidth ? 'w-full' : ''} group relative flex items-center gap-2 px-4 py-2.5 bg-[#1e1e2e]/80 hover:bg-[#262637] rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700`}
      >
        ...
        <span className="text-gray-300 flex-1 text-left group-hover:text-white transition-colors truncate">
          {currentTheme?.label}
        </span>

        {/* Color indicator */}
        <div className="relative w-4 h-4 rounded-full border border-gray-600 group-hover:border-gray-500 transition-colors flex-shrink-0"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute top-full ${fullWidth ? 'right-0' : 'left-0'} mt-2 w-full min-w-[200px] bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50`}
          >
            ...
            {THEMES.map((t, index) => (
              ...
              >
                ...
                {/* color indicator */}
                <div className="relative size-4 rounded-full border border-gray-600 group-hover:border-gray-500 transition-colors"
